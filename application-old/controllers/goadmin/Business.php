...
class Business extends CI_Controller 
{
	...
	// Also for table name
	var $url = 'business';
	var $model = 'model_business';
	var $title = 'Our Business';

	var $business_image = TRUE;

	var $thumb_width = 382;
	var $thumb_height = 292;

	var $image_width = 1123;
	...
	public function index()
	{
		...
		$filter = $this->input->post('business_filter');

		$where = array(
					'flag !=' => 3
		);

		if ($filter) $where['business_type'] = $filter;
...
//					'query'	=> $this->db->order_by($this->url.'_id', 'desc')->get_where($this->url, $where)
	...
	public function view($item_id)
	{
		...
		if ($check)
		{
			...
			if ($data['row'][$first]['business_image']) array_push($asset['js'], 'jquery.imgpreview.min', 'admin/imgpreview');
