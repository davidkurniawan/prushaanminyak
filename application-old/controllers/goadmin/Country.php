...
class Country extends CI_Controller {
	...
	public function view($country_id)
	{
		...
		$check = $this->db->get_where($this->url, array('unique_id' => $country_id, 'flag !=' => 3))->row_array();
		...
		if ($check)
		{
			...
			// $first = check_for_new_language($country_id, $this->url);
			...
			// 	}
			// }
			$data['row'] = $this->db->get_where($this->url, array('unique_id' => $country_id, 'flag !=' => 3, 'language_id' => 1))->row_array(); 
			$this->form_validation->set_rules('flag', 'flag', 'required');
