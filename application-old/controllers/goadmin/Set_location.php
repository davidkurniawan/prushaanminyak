...
class Set_location extends CI_Controller {
	...
	var $model = 'model_set_location';
	var $title = 'Set Location';
	...
	public function add()
	{
		...
		$asset = array(
					'title'	=> $this->title,
					'url'	=> $this->url,
					'js'	=> array('jquery.validate.min', 'admin/form', 'ckeditor/ckeditor', 'ckeditor/adapters/jquery', 'admin/ckeditor',),
					'css'	=> array()
				);
		

		$asset['list_phase']      = $this->db->get_where('phase', array('flag !=' => 3, 'language_id' => 1))->result_array();
		$asset['list_investment'] = $this->db->get_where('investment', array('flag !=' => 3, 'language_id' => 1))->result_array();
	...
	public function view($item_id)
	{
		...
		if ($check)
		{
			...
			// if ($data['row'][$first]['set_location_image']) array_push($asset['js'], 'jquery.imgpreview.min', 'admin/imgpreview');
