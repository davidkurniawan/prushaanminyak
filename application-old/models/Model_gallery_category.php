...
class Model_gallery_category extends CI_Model {
	...
	public function insert()
	{
		...
		foreach ($language as $lang_data)
		{
			$data = array(
					...
					'gallery_category_name'	=> $this->input->post('gallery_category_name_' . $lang_data['language_id']),
					'seo_url'		=> trim(url_title($this->input->post('gallery_category_name_' . $language[0]['language_id']), 'underscore', TRUE)),
					'gallery_category_type'	=> $this->input->post('gallery_category_type'),
					'gallery_category_parent' => $this->input->post('gallery_category_parent'),
