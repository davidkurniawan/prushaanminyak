...
class Model_investment extends CI_Model {
	...
	{
		// Upload Image
		$file = file_upload('investment_image', 'images/business/investment', FALSE);
		...
		foreach ($language as $lang_data)
		{
			$data = array(
				...
				'investment_name'		=> $this->input->post('investment_name_' . $lang_data['language_id']),
				'investment_image'		=> $file_name,
	...
	{
		// Upload Image
		$file = file_upload('investment_image', 'images/investment', FALSE);
		if ($file)
		{
			$image = image_resize($file, $this->image_width, $this->image_height);
			$file_name = $image['file_name'];
		...
		else
		{
			...
			$file_name = $row[0]['investment_image'];
		}
		// pre($row);
		$language = language()->result_array();
