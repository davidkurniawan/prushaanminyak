...
class Job_function extends CI_Controller {
	...
	// Also for table name
	var $url = 'job_function';
	var $model = 'Model_job_function';
	var $title = 'Job Function';
	...
	public function index()
	{
		...
		$asset['query'] = $this->db->order_by('unique_id', 'desc')->get_where($this->url, array('flag !=' => 3));
	...
	public function view($job_function_id)
	{
		check_access($this->url, 'read', TRUE);

		$check = $this->db->get_where($this->url, array('unique_id' => $job_function_id, 'flag !=' => 3))->row_array();
		...
		if ($check)
		{
			...
			// $first = check_for_new_language($job_function_id, $this->url);
			...
			// {
				...
				// {
			$data['row'] = $this->db->get_where($this->url, array('unique_id' => $job_function_id, 'flag !=' => 3, 'language_id' => 1))->row_array(); 
				// }
			// }
			$this->form_validation->set_rules('flag', 'flag', 'required');
