...
class Video extends CI_Controller {
	...
	public function add()
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			foreach (language()->result_array() as $find)
			{
				$asset['category'] = $this->db->order_by('unique_id', 'desc')->get_where('gallery_category', array('gallery_category_type' => 2, 'flag !=' => 3, 'language_id' => $find['language_id']));
	...
	public function view($item_id)
	{
		...
		if ($check)
		{
			...
			if ($this->form_validation->run() == FALSE)
			{
				...
				#print_r($asset['category']->result_array());
				#exit;
