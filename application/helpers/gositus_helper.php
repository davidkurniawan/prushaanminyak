	...
	function show_menu()
	{
		...
		foreach ($parent_modules->result_array() as $item)
		{
			...
			if ($item['module_id'] == 27)
			{
				...
				$unread = $ci->db->get_where('apply', array('apply_read' => 0,'flag !=' => 3))->num_rows();
...
if ( ! function_exists('file_upload'))
{
	function file_upload($field_name, $folder, $debug = FALSE)
	{
		...
		$config = array(
					'upload_path'	=> $folder,
					'allowed_types'	=> 'doc|pdf|txt|docx|xls|xlsx|gif|jpeg|jpg|png|register_tick_function(function)'
				);
