...
class Model_awards extends CI_Model {
...

	{
		...
		$file = file_upload('awards_image', 'images/awards', FALSE);

		$file = file_upload('awards_image', 'images/awards/thumb', FALSE);
...

		{
			...
			$data = array(
				...
				'awards_name'		=> $this->input->post('awards_name_' . $lang_data['language_id']),

				'seo_url'			=> trim(url_title($this->input->post('awards_name_' . $language[0]['language_id']), 'dash', TRUE)),

				'awards_type'		=> 1,

				'awards_image'		=> $file_name,

				'awards_image_thumb'=> $file_thumb,

				'awards_start'		=> $this->input->post('awards_start'),

				'awards_content'	=> $this->input->post('awards_content_' . $lang_data['language_id']),
...

	{
...

		{
			...
			$file_name = $row[0]['awards_image'];

			$file_thumb = $row[0]['awards_image_thumb'];
...

		{
			...
			$data = array(
					...
					'awards_end'		=> $this->input->post('awards_end'),
			...
			// if ($this->input->post('awards_type') == 1 || $this->input->post('awards_type') == 3) $data['awards_end'] = '2999-12-31';
