...
class Model_photo extends CI_Model {
	...
	{
		// Upload Image
		$file = file_upload('photo_image', 'images/photo', FALSE);
		...
		foreach ($language as $lang_data)
		{
			...
			$data = array(
					...
					'photo_name'	=> $this->input->post('photo_name_' . $lang_data['language_id']),
					'seo_url'		=> trim(url_title($this->input->post('photo_name_' . $language[0]['language_id']), 'underscore', TRUE)),
